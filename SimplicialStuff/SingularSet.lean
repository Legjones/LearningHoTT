...
instance {X : TopCat} : KanComplex (TopCat.toSSet.obj X) := by
  unfold KanComplex TopCat.toSSet --Presheaf.restricted... is representable type functor, ye? SimplexCategory.toTop is the thing which sends [n] to geometric n-simplex...
  unfold SimplexCategory.toTop SimplexCategory.toTop₀
  unfold IsFibrant
  intros
  sorry
...
/-- WANT TO DO:
(1) Use equivalence of Kan complex to horn filling thing... may need to fill in?
(2) Use that geometric simplices are contractible... find somewhere? (along with horns)
(3) Use that product of contractibles is contractible to see Horn × I is contractible
(4) Use that contractibles have maps between them, in particular homotopy equivalences (theorem ContractibleSpace.hequiv)
(5) Assemble:
      * Really want that horn is a retract of filled, if have then done (precompose)
      *

 Define projection from filled to horn... idea of projecting to make sure that point of horn
 as coordinate is always weighted in nonzero way. In particular


 (1) Kan complex equiv to filling horn condition (hopefully just simplify triangle quickly...)
 (2) Write down map from filled to horn which is constant/identity on horn.
 (3) See above is part of retraction, so in particular precomposition with this gives filling
 (4) Need to say that the "singular horn" maps correspond to the "simplicial horns" in SSet?

 Or do all of above in language of "simplicial horns"... unwrap what a "simplicial horn" is here...
 maybe use that have Quillen equivalence stuff with geometric realization and singular chains to see
 that corresponds to filling geometric maps...-/
